...
#Create and add FoodInsecure as Binary Y
...
#Lasso AUC = 0.63
#Ridge AUC = .64
#Go back and remake model with most impactful x's
...
# from the full models earlier but a little less. The other variables from the
#random forrest weren't very impactful
...
#Create lasso pred and lass_class column
...
head(acs_data[, c("lasso_pred", "lasso_class")])
...
final_lasso_coef
#education = -0.1887421
#for every increase in education FoodINsecure probibity goes down 18%
#married = -0.2677462
#Being married decreases 26%
roc_curve_1 <- roc(acs_data$lasso_class, acs_data$lasso_pred)  


head(acs_data[, c("lasso_pred", "lasso_class")])

## Seniors Pumas
seniors_puma <- read.csv("Final Project Data/total_iowa_seniors_by_puma.csv", stringsAsFactors = TRUE)
...
ggplot(data = joined) +
  geom_sf(aes(fill = lasso_class), color = "black", size = 0.2) +
  #scale_fill_brewer() +
  labs(
    title = "Food Insecurity by PUMA in Iowa"
  ) +
  theme_minimal()
